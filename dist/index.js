...
(() => {
  ...
  var distribute = function distribute2(v) {
    ...
    var vars = _isObject(v) ? v : {
      each: v
    }, ease = _parseEase(vars.ease), from = vars.from || 0, base = parseFloat(vars.base) || 0, cache3 = {}, isDecimal = from > 0 && from < 1, ratios = isNaN(from) || isDecimal, axis = vars.axis, ratioX = from, ratioY = from;
    if (_isString(from)) {
      ratioX = ratioY = {
        center: 0.5,
        edges: 0.5,
    ...
    return function(i, target, a) {
      var l = (a || vars).length, distances = cache3[l], originX, originY, x, y, d, j, max, min, wrapAt;
      if (!distances) {
  ...
  var _gsap = {
    ...
    quickSetter: function quickSetter(target, property, unit) {
      ...
      target = target[0] || {};
      var Plugin = _plugins[property], cache3 = _getCache(target), p = cache3.harness && (cache3.harness.aliases || {})[property] || property, setter = Plugin ? function(value) {
        var p2 = new Plugin();
        _quickTween._pt = 0;
      ...
      return Plugin ? setter : function(value) {
        return setter(target, p, unit ? value + unit : value, cache3, 1);
      };
    },
  ...
  var _setterScaleWithRender = function _setterScaleWithRender2(target, property, value, data, ratio) {
    var cache3 = target._gsap;
    cache3.scaleX = cache3.scaleY = value;
    cache3.renderTransform(ratio, cache3);
  };
  var _setterTransformWithRender = function _setterTransformWithRender2(target, property, value, data, ratio) {
    var cache3 = target._gsap;
    cache3[property] = value;
    cache3.renderTransform(ratio, cache3);
  };
  ...
  var _saveStyle = function _saveStyle2(property, isNotCSS) {
    var _this = this;
    var target = this.target, style = target.style, cache3 = target._gsap;
    if (property in _transformProps && style) {
      ...
      if (property !== "transform") {
        ...
        }) : this.tfm[property] = cache3.x ? cache3[property] : _get(target, property);
        property === _transformOriginProp && (this.tfm.zOrigin = cache3.zOrigin);
      } else {
  ...
  var _revertStyle = function _revertStyle2() {
    var props = this.props, target = this.target, style = target.style, cache3 = target._gsap, i, p;
    ...
    if (this.tfm) {
      ...
      if (cache3.svg) {
        cache3.renderTransform();
        target.setAttribute("data-svg-origin", this.svgo || "");
      }
      i = _reverting2();
      if ((!i || !i.isStart) && !style[_transformProp]) {
        _removeIndependentTransforms(style);
        if (cache3.zOrigin && style[_transformOriginProp]) {
          style[_transformOriginProp] += " " + cache3.zOrigin + "px";
          cache3.zOrigin = 0;
          cache3.renderTransform();
        }
        cache3.uncache = 1;
  ...
  var _convertToUnit = function _convertToUnit2(target, property, value, unit) {
    var curValue = parseFloat(value) || 0, curUnit = (value + "").trim().substr((curValue + "").length) || "px", style = _tempDiv.style, horizontal = _horizontalExp.test(property), isRootSVG = target.tagName.toLowerCase() === "svg", measureProperty = (isRootSVG ? "client" : "offset") + (horizontal ? "Width" : "Height"), amount = 100, toPixels = unit === "px", toPercent = unit === "%", px, parent, cache3, isSVG;
    ...
    cache3 = parent._gsap;
    if (cache3 && toPercent && cache3.width && horizontal && cache3.time === _ticker.time && !cache3.uncache) {
      return _round(curValue / cache3.width * amount);
    } else {
      ...
      if (horizontal && toPercent) {
        cache3 = _getCache(parent);
        cache3.time = _ticker.time;
        cache3.width = parent[measureProperty];
      }
    }
  ...
  var _renderClearProps = function _renderClearProps2(ratio, data) {
    if (data.tween && data.tween._time === data.tween._dur) {
      var target = data.t, style = target.style, props = data.u, cache3 = target._gsap, prop, clearTransforms, i;
      ...
      if (clearTransforms) {
        ...
        if (cache3) {
          cache3.svg && target.removeAttribute("transform");
  ...
  var _getMatrix = function _getMatrix2(target, force2D) {
    var cache3 = target._gsap || _getCache(target), style = target.style, matrix = _getComputedTransformMatrixAsArray(target), parent, nextSibling, temp, addedToDOM;
    if (cache3.svg && target.getAttribute("transform")) {
      temp = target.transform.baseVal.consolidate().matrix;
      matrix = [temp.a, temp.b, temp.c, temp.d, temp.e, temp.f];
      return matrix.join(",") === "1,0,0,1,0,0" ? _identity2DMatrix : matrix;
    } else if (matrix === _identity2DMatrix && !target.offsetParent && target !== _docElement && !cache3.svg) {
      temp = style.display;
      style.display = "block";
  ...
  var _applySVGOrigin = function _applySVGOrigin2(target, origin, originIsAbsolute, smooth, matrixArray, pluginToAddPropTweensTo) {
    var cache3 = target._gsap, matrix = matrixArray || _getMatrix(target, true), xOriginOld = cache3.xOrigin || 0, yOriginOld = cache3.yOrigin || 0, xOffsetOld = cache3.xOffset || 0, yOffsetOld = cache3.yOffset || 0, a = matrix[0], b = matrix[1], c = matrix[2], d = matrix[3], tx = matrix[4], ty = matrix[5], originSplit = origin.split(" "), xOrigin = parseFloat(originSplit[0]) || 0, yOrigin = parseFloat(originSplit[1]) || 0, bounds, determinant, x, y;
    if (!originIsAbsolute) {
    ...
    if (smooth || smooth !== false && cache3.smooth) {
      tx = xOrigin - xOriginOld;
      ty = yOrigin - yOriginOld;
      cache3.xOffset = xOffsetOld + (tx * a + ty * c) - tx;
      cache3.yOffset = yOffsetOld + (tx * b + ty * d) - ty;
    } else {
      cache3.xOffset = cache3.yOffset = 0;
    }
    cache3.xOrigin = xOrigin;
    cache3.yOrigin = yOrigin;
    cache3.smooth = !!smooth;
    cache3.origin = origin;
    cache3.originIsAbsolute = !!originIsAbsolute;
    target.style[_transformOriginProp] = "0px 0px";
    if (pluginToAddPropTweensTo) {
      _addNonTweeningPT(pluginToAddPropTweensTo, cache3, "xOrigin", xOriginOld, xOrigin);
      _addNonTweeningPT(pluginToAddPropTweensTo, cache3, "yOrigin", yOriginOld, yOrigin);
      _addNonTweeningPT(pluginToAddPropTweensTo, cache3, "xOffset", xOffsetOld, cache3.xOffset);
      _addNonTweeningPT(pluginToAddPropTweensTo, cache3, "yOffset", yOffsetOld, cache3.yOffset);
    }
    target.setAttribute("data-svg-origin", xOrigin + " " + yOrigin);
  };
  var _parseTransform = function _parseTransform2(target, uncache) {
    var cache3 = target._gsap || new GSCache(target);
    if ("x" in cache3 && !uncache && !cache3.uncache) {
      return cache3;
    }
    var style = target.style, invertedScaleX = cache3.scaleX < 0, px = "px", deg = "deg", cs = getComputedStyle(target), origin = _getComputedProperty(target, _transformOriginProp) || "0", x, y, z, scaleX, scaleY, rotation, rotationX, rotationY, skewX, skewY, perspective, xOrigin, yOrigin, matrix, angle, cos, sin, a, b, c, d, a12, a22, t1, t2, t3, a13, a23, a33, a42, a43, a32;
    x = y = z = rotation = rotationX = rotationY = skewX = skewY = perspective = 0;
    scaleX = scaleY = 1;
    cache3.svg = !!(target.getCTM && _isSVG(target));
    if (cs.translate) {
    ...
    matrix = _getMatrix(target, cache3.svg);
    if (cache3.svg) {
      if (cache3.uncache) {
        t2 = target.getBBox();
        origin = cache3.xOrigin - t2.x + "px " + (cache3.yOrigin - t2.y) + "px";
        t1 = "";
      } else {
        t1 = !uncache && target.getAttribute("data-svg-origin");
      }
      _applySVGOrigin(target, t1 || origin, !!t1 || cache3.originIsAbsolute, cache3.smooth !== false, matrix);
    }
    xOrigin = cache3.xOrigin || 0;
    yOrigin = cache3.yOrigin || 0;
    if (matrix !== _identity2DMatrix) {
      ...
      if (cache3.svg) {
        ...
        cache3.forceCSS = target.setAttribute("transform", "") || !_isNullTransform(_getComputedProperty(target, _transformProp));
    ...
    uncache = uncache || cache3.uncache;
    cache3.x = x - ((cache3.xPercent = x && (!uncache && cache3.xPercent || (Math.round(target.offsetWidth / 2) === Math.round(-x) ? -50 : 0))) ? target.offsetWidth * cache3.xPercent / 100 : 0) + px;
    cache3.y = y - ((cache3.yPercent = y && (!uncache && cache3.yPercent || (Math.round(target.offsetHeight / 2) === Math.round(-y) ? -50 : 0))) ? target.offsetHeight * cache3.yPercent / 100 : 0) + px;
    cache3.z = z + px;
    cache3.scaleX = _round(scaleX);
    cache3.scaleY = _round(scaleY);
    cache3.rotation = _round(rotation) + deg;
    cache3.rotationX = _round(rotationX) + deg;
    cache3.rotationY = _round(rotationY) + deg;
    cache3.skewX = skewX + deg;
    cache3.skewY = skewY + deg;
    cache3.transformPerspective = perspective + px;
    if (cache3.zOrigin = parseFloat(origin.split(" ")[2]) || !uncache && cache3.zOrigin || 0) {
      style[_transformOriginProp] = _firstTwoOnly(origin);
    }
    cache3.xOffset = cache3.yOffset = 0;
    cache3.force3D = _config.force3D;
    cache3.renderTransform = cache3.svg ? _renderSVGTransforms : _supports3D ? _renderCSSTransforms : _renderNon3DTransforms;
    cache3.uncache = 0;
    return cache3;
  };
  ...
  var _renderNon3DTransforms = function _renderNon3DTransforms2(ratio, cache3) {
    cache3.z = "0px";
    cache3.rotationY = cache3.rotationX = "0deg";
    cache3.force3D = 0;
    _renderCSSTransforms(ratio, cache3);
  };
  var _zeroDeg = "0deg";
  var _zeroPx = "0px";
  var _endParenthesis = ") ";
  var _renderCSSTransforms = function _renderCSSTransforms2(ratio, cache3) {
    var _ref = cache3 || this, xPercent = _ref.xPercent, yPercent = _ref.yPercent, x = _ref.x, y = _ref.y, z = _ref.z, rotation = _ref.rotation, rotationY = _ref.rotationY, rotationX = _ref.rotationX, skewX = _ref.skewX, skewY = _ref.skewY, scaleX = _ref.scaleX, scaleY = _ref.scaleY, transformPerspective = _ref.transformPerspective, force3D = _ref.force3D, target = _ref.target, zOrigin = _ref.zOrigin, transforms = "", use3D = force3D === "auto" && ratio && ratio !== 1 || force3D === true;
  ...
  var _renderSVGTransforms = function _renderSVGTransforms2(ratio, cache3) {
    var _ref2 = cache3 || this, xPercent = _ref2.xPercent, yPercent = _ref2.yPercent, x = _ref2.x, y = _ref2.y, rotation = _ref2.rotation, skewX = _ref2.skewX, skewY = _ref2.skewY, scaleX = _ref2.scaleX, scaleY = _ref2.scaleY, target = _ref2.target, xOrigin = _ref2.xOrigin, yOrigin = _ref2.yOrigin, xOffset = _ref2.xOffset, yOffset = _ref2.yOffset, forceCSS = _ref2.forceCSS, tx = parseFloat(x), ty = parseFloat(y), a11, a21, a12, a22, temp;
  ...
  var CSSPlugin = {
    ...
    init: function init3(target, vars, tween, index, targets) {
      var props = this._props, style = target.style, startAt = tween.vars.startAt, startValue, endValue, endNum, startNum, type, specialProp, p, startUnit, endUnit, relative, isTransformRelated, transformPropTween, cache3, smooth, hasPriority, inlineProps;
      _pluginInitted || _initCore();
        ...
        if (specialProp) {
          ...
          if (isTransformRelated) {
            ...
            if (!transformPropTween) {
              cache3 = target._gsap;
              cache3.renderTransform && !vars.parseTransform || _parseTransform(target, vars.parseTransform);
              smooth = vars.smoothOrigin !== false && cache3.smooth;
              transformPropTween = this._pt = new PropTween(this._pt, style, _transformProp, 0, 1, cache3.renderTransform, cache3, 0, -1);
              transformPropTween.dep = 1;
            }
            if (p === "scale") {
              this._pt = new PropTween(this._pt, cache3, "scaleY", cache3.scaleY, (relative ? _parseRelative(cache3.scaleY, relative + endNum) : endNum) - cache3.scaleY || 0, _renderCSSProp);
              this._pt.u = 0;
              props.push("scaleY", p);
              p += "X";
              ...
              if (cache3.svg) {
                ...
                endUnit !== cache3.zOrigin && _addNonTweeningPT(this, cache3, "zOrigin", cache3.zOrigin, endUnit);
              ...
              _addRotationalPropTween(this, cache3, p, startNum, relative ? _parseRelative(startNum, relative + endValue) : endValue);
              continue;
            } else if (p === "smoothOrigin") {
              _addNonTweeningPT(this, cache3, "smooth", cache3.smooth, endValue);
              continue;
            } else if (p === "force3D") {
              cache3[p] = endValue;
              continue;
          ...
          if (isTransformRelated || (endNum || endNum === 0) && (startNum || startNum === 0) && !_complexExp.test(endValue) && p in style) {
            ...
            this._pt = new PropTween(this._pt, isTransformRelated ? cache3 : style, p, startNum, (relative ? _parseRelative(startNum, relative + endNum) : endNum) - startNum, !isTransformRelated && (endUnit === "px" || p === "zIndex") && vars.autoRound !== false ? _renderRoundedCSSProp : _renderCSSProp);
  ...
  var _swapPinOut = function _swapPinOut2(pin, spacer, state) {
    _setState(state);
    var cache3 = pin._gsap;
    if (cache3.spacerIsNative) {
      _setState(cache3.spacerState);
  ...
  var _nestedScroll = function _nestedScroll2(_ref5) {
    ...
    var node = (event.changedTouches ? event.changedTouches[0] : event).target, cache3 = node._gsap || gsap3.core.getCache(node), time = _getTime2(), cs;
    if (!cache3._isScrollT || time - cache3._isScrollT > 2e3) {
      while (node && node !== _body2 && (node.scrollHeight <= node.clientHeight && node.scrollWidth <= node.clientWidth || !(_overflow[(cs = _getComputedStyle(node)).overflowY] || _overflow[cs.overflowX]))) {
        node = node.parentNode;
      }
      cache3._isScroll = node && node !== target && !_isViewport3(node) && (_overflow[(cs = _getComputedStyle(node)).overflowY] || _overflow[cs.overflowX]);
      cache3._isScrollT = time;
    }
    if (cache3._isScroll || axis === "x") {
  ...
  // src/jords-co/blurInElements.ts
  var blurInElements = () => {
  ...
  function parseTargets(targets) {
    var targetsArray = targets ? flattenArray(is.arr(targets) ? targets.map(toArray4) : toArray4(targets)) : [];
  ...
  // src/jords-co/updateProjectInfo.ts
  ...
  // src/jords-co/homeHeroHeadingImages.ts
  var homeHeroHeadingImages = () => {
    ...
    sortedElements.forEach((element) => {
      if (element.tagName === "DIV") {
        ...
        splitElements.push(split2.chars);
      }
    });
    ...
    gsapWithCSS.from(splitElements, {
      ease: "ease",
      duration: 1,
      y: 20,
      blur: 10,
